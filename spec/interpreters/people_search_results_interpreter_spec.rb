...
describe PeopleSearchResultsInterpreter do
  ...
  describe '.interpret_source_ssn' do
    it 'redacts all but last four characters of ssn' do
      document = { '_source' => { 'ssn' => '123456789' } }
      described_class.interpret_source_ssn(document)
      expect(document).to match a_hash_including(
        '_source' => a_hash_including(
          'ssn' => '___-__-6789'
        )
    ...
    it 'redacts all but last four characters of ssn highlight inside _source' do
      document = { '_source' => { 'highlight' => { 'ssn' => '<em>123456789</em>' } } }
      described_class.interpret_source_ssn(document)
      expect(document).to match a_hash_including(
        '_source' => a_hash_including(
          'highlight' => { 'ssn' => '<em>6789</em>' }
        )
      )
    end
  end
